...
Page({
  ...
  /*点击切换导航的回调*/
  changeNav(event) {
    let navId = event.currentTarget.id;//通过id向event传参的时候如果传的是number会自动转成string
    this.setData({
      navId: navId * 1,
      videoList: []
  ...
  handlePlay(event) {
    ...
    * */
    ...
    //this.vid = vid;
    //更新data中的videoId
    //创建控制video标签的实例对象的状态数据
    this.setData({
      videoId: vid
    })
    this.videoContext = wx.createVideoContext(vid);
    //判断当前视频是否有播放过，如果有跳转到上一次播放位置
  ...
  onShareAppMessage: function ({from}) {
    ...
    if (from === 'button') {
      return {
        ...
        imageUrl: '/static/images/tuijian1.png'
      }
    } else {
      return {
        title: '来自menu的转发',
